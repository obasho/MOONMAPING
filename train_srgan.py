...
import model
import srgan_config
from dataset import CUDAPrefetcher, TrainValidImageDataset, TestImageDataset, TrainCustomDataset
from image_quality_assessment import PSNR, SSIM, SAM
...
def main():
    ...
    if srgan_config.resume_d_model_weights_path:
        d_model, _, start_epoch, best_psnr, best_ssim, best_sam, optimizer, scheduler = load_state_dict(
            d_model,
    ...
    if srgan_config.resume_g_model_weights_path:
        g_model, _, start_epoch, best_psnr, best_ssim, best_sam, optimizer, scheduler = load_state_dict(
            g_model,
    ...
    sam_model = SAM() #srgan_config.upscale_factor, srgan_config.only_test_y_channel)

    # Transfer the IQA model to the specified device
    psnr_model = psnr_model.to(device=srgan_config.device)
    ssim_model = ssim_model.to(device=srgan_config.device)
    sam_model = sam_model.to(device=srgan_config.device)
    ...
    for epoch in range(start_epoch, srgan_config.epochs):
        train(d_model,
              ...
              epoch,
              writer)
        psnr, ssim, sam = validate(g_model,
                              test_prefetcher,
                              epoch,
                              writer,
                              psnr_model,
        ...
        is_best = sam < best_sam
        is_last = (epoch + 1) == srgan_config.epochs
        best_psnr = max(psnr, best_psnr)
        best_ssim = max(ssim, best_ssim)
        best_sam = min(sam,best_sam)
...
def load_dataset():
    ...
    train_datasets = TrainCustomDataset(srgan_config.train_gt_images_dir,
                                        srgan_config.train_lr_images_dir,
                                        srgan_config.gt_image_size,
                                        srgan_config.lr_image_size,
...
def validate(
    ...
    sames = AverageMeter("SAM", ":4.6f")
    progress = ProgressMeter(len(data_prefetcher), [batch_time, psnres, ssimes, sames], prefix=f"{mode}: ")
    ...
    with torch.no_grad():
        while batch_data is not None:
            ...
            sames.update(sam.item(), lr.size(0))
    ...
    if mode == "Valid" or mode == "Test":
        ...
        writer.add_scalar(f"{mode}/SAM", sames.avg, epoch + 1)
    else:
        raise ValueError("Unsupported mode, please use `Valid` or `Test`.")

    return psnres.avg, ssimes.avg, sames.avg
